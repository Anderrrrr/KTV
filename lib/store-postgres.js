// Online storage: Postgres (Neon, via KTV_DATABASE_URL). Many rooms, each with its own queue and
...
export function createPostgresStore(database) {
  ...
  return {
    ...
    async state(roomId) {
      const list = await rows(`SELECT q.id, q.status, q.added_by, q.created_at, q.use_partner, s.id AS seen_id,
    ...
    async setUsePartner(roomId, queueId, on) {
      return (await rows("UPDATE queue SET use_partner=$1 WHERE id=$2 AND room_id=$3 AND status='playing' RETURNING id", [on, queueId, roomId])).length > 0;
    },
    partnerOf(seenId) {
      return one(`SELECT s.id AS seen_id, s.title, s.video_id, p.offset_ms FROM song_partner p
          JOIN seen s ON s.id=p.partner_id WHERE p.seen_id=$1`, [seenId]);
    },
    async savePartner(seenId, partnerId, offsetMs) {
      await withClient(async client => {
        const upsert = `INSERT INTO song_partner(seen_id,partner_id,offset_ms) VALUES($1,$2,$3)
        ...
        try {
          ...
          await client.query('DELETE FROM song_partner WHERE seen_id IN ($1,$2) OR partner_id IN ($1,$2)', [seenId, partnerId]);
          await client.query(upsert, [seenId, partnerId, offsetMs]);
          await client.query(upsert, [partnerId, seenId, -offsetMs]);
          await client.query('COMMIT');
        } catch (error) { await client.query('ROLLBACK'); throw error; }
      });
    },
    async setPartnerOffset(seenId, offsetMs) {
      const updated = await rows(`WITH mine AS (UPDATE song_partner SET offset_ms=$2 WHERE seen_id=$1 RETURNING partner_id)
          UPDATE song_partner SET offset_ms=-$2 WHERE seen_id=(SELECT partner_id FROM mine) RETURNING seen_id`, [seenId, offsetMs]);
      return updated.length > 0;
    },
    seenWithPrefix(prefix) {
      return rows("SELECT id, title, video_id FROM seen WHERE title ILIKE $1 ESCAPE '\\' ORDER BY id LIMIT 30",
        [`${prefix.replace(/[\\%_]/g, '\\$&')}%`]);
    },
    async findSeenByVideo(videoId) {
      return (await one('SELECT id FROM seen WHERE video_id=$1 ORDER BY id LIMIT 1', [videoId]))?.id ?? null;
    },
