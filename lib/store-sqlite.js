...
// 我懷念的－純伴奏 was made private on YouTube.
const REPLACED_VIDEOS = { '4n8KJ8nRSEk': 'UHS8kRuWGq4' };
...
CREATE TABLE IF NOT EXISTS lyrics (
  ...
  // Starter songs whose YouTube video later became unplayable, and their replacements.
  // Existing catalogues get the fix on start; pairing offsets for the song reset since timing differs.
  for (const [dead, replacement] of Object.entries(REPLACED_VIDEOS)) {
    const row = db.prepare('SELECT id FROM seen WHERE video_id=?').get(dead);
    if (!row) continue;
    db.prepare('UPDATE seen SET video_id=?, youtube_url=? WHERE id=?').run(replacement, `https://www.youtube.com/watch?v=${replacement}`, row.id);
    db.prepare('UPDATE song_partner SET offset_ms=0 WHERE seen_id=? OR partner_id=?').run(row.id, row.id);
  }
